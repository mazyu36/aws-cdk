  ...
  return {
    llmExtractionConfig: {
      ...
      validation: config.llmExtractionConfig.validation
        ? renderValidation(config.llmExtractionConfig.validation)
        : undefined,
    },
  };
}
...
): bedrockagentcore.CfnMemory.ValidationProperty {
  return {
    stringValidation: validation.stringValidation,
    stringListValidation: validation.stringListValidation,
...
function validateMetadataSchemaEntry(entry: MetadataSchemaEntry, scope?: IConstruct): string[] {
  ...
  if (entry.extractionConfig?.llmExtractionConfig) {
    errors.push(...validateLlmExtractionConfig(entry.extractionConfig.llmExtractionConfig, scope));
  }
  return errors;
}

function validateLlmExtractionConfig(config: LlmExtractionConfig, _scope?: IConstruct): string[] {
  const errors: string[] = [];
  if (!Token.isUnresolved(config.definition) && config.definition.length === 0) {
